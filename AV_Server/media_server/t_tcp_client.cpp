...
ConnAttr::ConnAttr(int iFd, unsigned int iSeq)
    ...
    m_iClientStatus(0),m_pTcpClientHandle(NULL)
{
...
{
    TcpClient* pTcpClient = (TcpClient*)arg;
...
void ConnAttr::IoWrite(int iFd)
{
    ...
    if ( m_iClientStatus == 1 )
    {
        ...
        if ( iRet != 0 || iSockErr )
        {
            MTS_LOG_ERROR("getsockopt failed (errno %d)!, async connect sock err: %d ", 
                          errno, iSockErr);
            m_pTcpClientHandle->DestroyConn(this);
            return ;
        }
    ...
    if (iWritenLen < 0)
    {
        ...
        bool bRet = AddEvent( m_pTcpClientHandle->GetEventBase(), EV_WRITE, m_pTcpClientHandle);
        if (bRet == false)
        {
    ...
    else if (iWritenLen >0)
    {
        ...
        else 
        {
            ...
            AddEvent( m_pTcpClientHandle->GetEventBase(), EV_WRITE, m_pTcpClientHandle );
        }
    }
    else
    {
...
////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////
TcpClient::TcpClient( struct event_base* pEventBase )
    :base( pEventBase )
{
}
...
bool TcpClient::Send(const std::string& sIp, unsigned short usPort,
...
bool TcpClient::AutoSend(const std::string& sIp, unsigned short usPort,
                         rapidjson::Document& iRoot)
{
...
bool TcpClient::Send(const tagConnFd& conFd, rapidjson::Document& iRoot)
{
    int iFd  = conFd.iFd;
...
std::shared_ptr<ConnAttr> TcpClient::CreateConnAttr(int iFd, unsigned int uiSeq)
{
    std::shared_ptr<ConnAttr>  oneconnAttr(new ConnAttr(iFd, uiSeq));
    return oneconnAttr;
}

bool TcpClient::EncodeSendData( std::shared_ptr<ConnAttr> pConnAttr, 
                                rapidjson::Document& iRoot )
{
    return pConnAttr->EncodeData(iRoot);
}

void TcpClient::DestroyConn(ConnAttr* pConnAttr)
{
    if (!pConnAttr)
    {
...
bool TcpClient::AddWriteEvent(std::shared_ptr<ConnAttr> pConnAttr)
{
    struct event_base* pBase = base;
    if (pBase == NULL)
    {
...
bool TcpClient::AddReadEvent(std::shared_ptr<ConnAttr> pConnAttr)
{
    struct event_base* pBase = base;
    if (pBase == NULL)
    {
