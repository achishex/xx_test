...
AcceptConn::AcceptConn(int ifd, void* pData)
    : ConnBase(ifd), m_pData((MtsTcp*)pData), 
    m_pCodec(NULL)
{
...
{
    MTS_LOG_DEBUG("accept fd: %d begin to recv data", m_iFd);

    int iErrNo = 0;
    pRecvBuff->Compact(8*1024);
    int iRet =  pRecvBuff->ReadFD(m_iFd, iErrNo);

    MTS_LOG_DEBUG("recv from fd: %d data len: %d, ReadableBytes() = %d, read_index: %d",
    ...
    if (iRet < 0)
    {
        ...
        {
            MTS_LOG_ERROR("recv err from client, errmsg: %s", strerror(iErrNo));
    ...
    if (iRet == 0)
    {
        MTS_LOG_INFO("peer client active close socket fd: %d", m_iFd);
        m_pData->DeleteAcceptConn(m_iFd);
        return true;
    }
    ...
    {
        MTS_LOG_DEBUG("not complete recv data on fd: %d", m_iFd);
        return true;
    }
    else if ( codeRet == RET_RECV_OK )
    {
        ...
        do {
            ...
            {
                //build resp json 
                BuildErrResp(jsonDataResp, jsonData, -1, "method field value empty");
                MTS_LOG_ERROR("method field val is empty");
                break;
            }
            MTS_LOG_DEBUG("client req method: %s", method_val.c_str());
            ...
            if (!pBusiModule)
            {
                ...
                MTS_LOG_ERROR("not get busi module for method: %s", method_val.c_str());
                break;
            }
        ...
        {
            m_pData->DeleteAcceptConn(m_iFd);
            MTS_LOG_ERROR("encode response fail on fd: %d", m_iFd);
            return false;
        }
    ...
    {
        MTS_LOG_ERROR("parse recv data fail, on fd: %d", m_iFd);
        m_pData->DeleteAcceptConn(m_iFd);
    }
    else 
    {
...
bool AcceptConn::DoWrite()
{
    ...
    if (iWriteLen < 0)
    {
        ...
        {
            MTS_LOG_ERROR("send to fd: %d, err msg: %s", m_iFd, strerror(iErrNo));
            m_pData->DeleteAcceptConn(m_iFd);
            return false;
        }
        ...
        if (EAGAIN == iErrNo)
        {
            MTS_LOG_DEBUG("send to fd go on, fd: %d", m_iFd);
            AddEvent(m_pData->GetEventBase(), EV_WRITE, this);
            return true;
        }
        else
        {
            MTS_LOG_DEBUG("send to fd interrupt, fd: %d", m_iFd);
            AddEvent(m_pData->GetEventBase(), EV_WRITE, this);
            return true;
        }
    }
    else if (iWriteLen > 0)
    {
        ...
        {
            MTS_LOG_DEBUG("send data len: %d to peer ok on fd: %d", iWriteLen, m_iFd);
        }
        else 
        {
            MTS_LOG_DEBUG("has write data len: %d, to write data len: %d, on fd: %d",
...
std::shared_ptr<PolicyBusi> AcceptConn::GetBusiModule(const std::string& sMethod)
{
    ...
    auto itBusiModule = m_pData->m_mpBusiModule.find(sMethod);
    if (itBusiModule != m_pData->m_mpBusiModule.end())
    {
        return itBusiModule->second;
    }
    else 
    {
        MTS_LOG_ERROR("not find method process obj, method: %s", sMethod.c_str());
        return tmpRet; 
    }
}
...
std::string AcceptConn::GetKeyItemVal(const std::string& sKey, rapidjson::Document &root)
{
    ...
    {
        MTS_LOG_ERROR("json root not obj ");
        return std::string("");
    }
    if ( !root.HasMember(sKey.c_str()) || !root[sKey.c_str()].IsString() )
    {
        MTS_LOG_ERROR("json root has not field: %s, or field: %s val not string format", 
                              ...
                              int iCode, const std::string& sErrMsg)
{
    ...
    do {
        ...
        {
            MTS_LOG_ERROR("req json parse err");
        ...
        if ( !inroot.IsObject() )
        {
            MTS_LOG_ERROR("req json is not obj");
