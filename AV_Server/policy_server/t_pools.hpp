...
namespace T_TCP
{
    ...
    template<typename T>
    PthreadPools<T>::PthreadPools(const int iPoolSize)
    ...
    bool PthreadPools<T>::StartAllThreads()
    {
        ...
        WaitThreadRegiste();
        PL_LOG_INFO("all threads start ok, work threads nums:%d, manager thread nums: %d",
                    m_iPoolSize -1 , 1);
        return true;
    }
    ...
    template<typename T>
    T* PthreadPools<T>::GetOneThread(int iIndex)
    {
